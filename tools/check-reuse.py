...
WHY IT IS A LOCAL / PRE-RELEASE GATE, NOT PART OF check-static
    ...
    The device-free CI aggregate (make check-static, run by the GitLab pipeline
    on a plain `python:3-slim` image) is deliberately kept free of any pip
    install step, so it must not depend on `reuse` being present. This gate
