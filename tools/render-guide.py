...
-----
    ...
    python3 tools/render-guide.py --make-deps  # print a Makefile fragment wiring
                                             # each output to its template +
                                             # strings.xml (for `include` by make)
"""
...
def emit_make_deps(langs) -> int:
    ...
    For every language it emits one prerequisite-only rule
        ...
        app/src/main/res/<raw_dir>/usersguide.md: <template>.md.in \\
            app/src/main/res/<values_dir>/strings.xml ../tools/render-guide.py

    plus a ``GUIDE_OUTPUTS`` variable listing all outputs. Paths are relative to
    the ``android/`` directory (where ``make`` runs), so android/Makefile can
    ``include`` the fragment. Only PREREQUISITES are emitted -- the recipe is
    supplied by android/Makefile's single shared guide rule, so make owns the
    "rebuild only when the template or strings.xml changed" decision while the
    LANGUAGE SET stays discovered here from the template files (one source of
    truth): adding a ``usersguide.xx.md.in`` regenerates the fragment and the
    matching rule appears automatically.
    """
    renderer = os.path.relpath(os.path.abspath(__file__), ROOT)
    outputs, rules = [], []
    for _label, tpl_path, values_dir, raw_dir in langs:
        out = os.path.relpath(os.path.join(RES, raw_dir, "usersguide.md"), ROOT)
        tpl = os.path.relpath(tpl_path, ROOT)
        strings = os.path.relpath(os.path.join(RES, values_dir, "strings.xml"), ROOT)
        outputs.append(out)
        rules.append(f"{out}: {tpl} {strings} {renderer}")
    print("# Auto-generated by `render-guide.py --make-deps`; do not edit or commit.")
    print("GUIDE_OUTPUTS := " + " ".join(outputs))
    print("\n".join(rules))
    return 0
...
def main() -> int:
    ...
    # must be identical (see check_language_parity). Runs in every mode so a
    # normal build, CI and the dependency-fragment generation all fail fast on
    # any drift.
