...
check_room_migrations() {
    ...
    if [[ -f "$schema_file" ]]; then
        # Existence alone proved little: any file under the right name passed,
        # including one left over from an earlier version. The exported schema
        # states its own version, so that claim is checked against @Database.
        local exported_version
        exported_version=$(sed -n 's/.*"version"[[:space:]]*:[[:space:]]*\([0-9]\+\).*/\1/p' "$schema_file" | head -1)
        if [[ -z "$exported_version" ]]; then
            fail "Schema file $schema_file carries no \"version\" — it is not a Room export"
        elif [[ "$exported_version" != "$db_version" ]]; then
            fail "Schema file $schema_file says version $exported_version, @Database says $db_version — regenerate it"
        else
            pass "Schema file $schema_file exists and states version $db_version"
        fi
    else
