...
Libellus Potionis needs Android 11 or later, which covers phones
from roughly 2020 onwards. It is tested regularly on a Google Pixel 10 Pro
running GrapheneOS (Android 16) and on a Fairphone 4 (Android
15).
...
bottom right and opens the entry
sheet.
...
As soon as the volume is in range, the sheet tells you how many grams of pure alcohol that comes
...
them, and an edit and a delete icon after them. The
star toggles the favorite, which is what puts a drink in the quick buttons
on the Today screen.

Tapping the row itself - not the icons - opens the entry sheet
for that drink, which is the second way to log, alongside the plus button on
Today. The two icons on the right open the editor and the delete
prompt.
...
- **Alcohol Content** - the usual strength, from 0 to 100 percent.
...
bottom right, which only appears
...
period at the top - 7 Days, Month or Year - and everything below
...
The Data Export section of the Statistics screen offers two formats.

**CSV** writes a table with one row per entry, ready for a
...
**PDF** produces a two-page report: the headline figures, a month
...
The CSV lands in your Downloads folder, and the app offers to
share it afterwards. The PDF goes to the system print dialog instead, from
where you can print it or save it as a
file.
...
no device-to-device transfer. **When you change phones, factory-reset, or
reinstall, a backup you made yourself is the only thing that carries your
data across.** Whatever you did not export is
gone.
...
**Biometric Lock** asks for a fingerprint, a face, or your
device lock whenever you open the
app. Turning the lock on or off
