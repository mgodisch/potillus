...
Top right on all four screens sits the same “burger” menu button. It opens a menu
...
further ones. At the bottom right
...
A tap on an entry’s pencil icon in the day’s list opens it as _Edit Entry_: the same fields
as when you logged it, and the same bounds. The trash icon removes
it, and the app asks once before it does, because an
...
dot for that serving ahead of them, and an edit and a delete icon
after them. The star toggles the favorite: favorites appear in
the quick buttons on the _Today_ screen.

A tap on the row (not on the icons) opens the entry sheet for
that drink; this is the third way to log, alongside the plus buttons on the
_Today_ and the _Calendar_ screens. The two icons on the right open
the editor and the delete prompt.

### Adding or changing a drink

The plus button opens _Add Drink_, a row’s pencil
icon opens _Edit Drink_.
...
- _Alcohol Content_: the usual strength, from 0 to 100 percent.
...
The _CSV_ button on the _Statistics_ screen writes a table
...
data.** Whatever was not exported is gone then.
...
- _Biometric Lock_ asks, whenever the app is opened, for a
  fingerprint, a face, or the device lock. Turning the lock on or off requires that
