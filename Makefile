...
#    Housekeeping
#      clean-android / distclean-android   clear the Android build output /
#                                          also its generated sources
#      clean-ios / distclean-ios           clear the iOS build output /
#                                          also its generated sources
#                                          (none of them touch releases/)
# =============================================================================
...
# android/app/build/ (git-ignored, cleared by `make clean-android`) and — because
...
ios-project: ios-version ios/Potillus/Resources/license_gpl3.md ios-guides
	command -v xcodegen
...
# Generated (gitignored) like license_gpl3.md above; the app picks the file for its
...
# One pair of targets PER PLATFORM, following the -android/-ios convention the
# rest of this file uses (release-android/release-ios, screenshots-android/
# screenshots-ios). There is deliberately no plain `clean`/`distclean` any more:
# the single pair only ever cleaned Android -- the entire iOS tree was left
# standing -- so its name promised twice what it delivered. Two honest halves beat
# one dishonest whole, and `make clean-ios` says which tree it clears.
#
# The clean/distclean SPLIT is the one android/Makefile already draws, kept here
# so the two platforms mean the same thing by the same word:
...
#                 start (the Xcode project, the version xcconfig, the copied
#                 license and rendered guides), i.e. back to a fresh-clone state.
#
# What NONE of them touch is releases/: those are staged release artifacts that
# push-appstore/push-playstore verify and upload, and release-ios deliberately
# refuses to overwrite one. Clearing them is a decision, never a side effect of
...
# The iOS build outputs. ios/build/ holds the xcarchive, the exported .ipa, the
# generated ExportOptions.plist and the SBOM (all from release-ios); the two
# PotillusKit directories are SwiftPM's own caches from `swift test`. Every path
# here is named in ios/.gitignore -- that file is the inventory this target
# clears, and the reason a `rm -rf` of them is safe.
clean-ios:
	rm -rf ios/build
	rm -rf ios/PotillusKit/.build ios/PotillusKit/.swiftpm

# ...plus everything a build GENERATES into the source tree. Each is produced by a
# target named beside it, so a distclean-ios costs exactly one `make ios-project`
# to undo -- and each is git-ignored, which is why removing them loses nothing.
distclean-ios: clean-ios
	rm -f ios/Version.xcconfig                       # make ios-version
	rm -rf ios/Potillus.xcodeproj                    # make ios-project (xcodegen)
	rm -f ios/Potillus/Resources/license_gpl3.md     # make ios-project (copy of LICENSE.md)
