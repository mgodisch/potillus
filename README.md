...
on the [App Store (Beta)](https://testflight.apple.com/join/sfJvr3VK). While
...
* Adjust the app to set your own "day start" time, so that late-night drinks
  ...
  clean restart.
* Screen-reader names on every interactive control, text that follows the
  system font size, right-to-left layouts, and a limit palette that reads
  without telling red from green.
* An in-app User's Guide describes every screen.
...
synchronization. The app's full privacy policy — detailing exactly what is
stored on the device and confirming that nothing is ever transmitted — is
available in [`PRIVACY.md`](PRIVACY.md).

## Platform Compatibility

Libellus Potionis runs on Android 11 (API 30) and newer — a Pixel 5 or Galaxy
S21 and later — and on iOS 17 and newer, an iPhone XS (2018) and later. It
...
set of test vectors. To build the app from source, follow the step-by-step
guides [`docs/INSTALL-ANDROID.md`](docs/INSTALL-ANDROID.md) and
[`docs/INSTALL-IOS.md`](docs/INSTALL-IOS.md). Changes are documented in
[`CHANGELOG.md`](CHANGELOG.md). The project's intended direction and its
explicit non-goals are described in [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
reporting process described in [`SECURITY.md`](SECURITY.md). Translation
contributions are where help is most useful: of the 21 interface languages,
only English and German are hand-written, and the rest have never been read by
a native speaker. Which are which, and how to send a correction, is in
[`CONTRIBUTING.md`](CONTRIBUTING.md), section 6. All contributors are expected
to follow the project's [Code of Conduct](docs/CODE_OF_CONDUCT.md).

## License

Libellus Potionis - Privacy-Friendly Alcohol Tracker, Copyright &copy; 2026
Martin A. Godisch <[martin@godisch.de](mailto:martin@godisch.de)>. The source
code can be found at the [canonical repository at
...
cannot — see [`docs/MIRROR-CHECKS.md`](docs/MIRROR-CHECKS.md). This program is
