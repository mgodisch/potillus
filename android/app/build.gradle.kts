...
android {
    ...
    packaging {
        resources {
            ...
            // copy of LICENSE.Apache-2.0.md; see generateLicenceDocuments below).
...
// Copies each verbatim licence text the app must reproduce into res/raw, one
// file per licence, via tools/render-copyright.py — the SAME generator the
// Makefile rules and their check-guides verification use, so the two build paths
// cannot disagree about the generated bytes. With a single input the script
// emits the source verbatim; what it adds over `cp` is LF line endings pinned on
// every platform.
//
//   license_gpl3.md    the app's own licence, linked from the About screen
...
val generateLicenceDocuments = licenceDocuments.map { (name, source, what) ->
    tasks.register<Exec>("generate${name.replaceFirstChar { it.uppercase() }}Document") {
        description = "Build res/raw/$name.md from $source ($what) via tools/render-copyright.py."
        ...
        val output = layout.projectDirectory.file("src/main/res/raw/$name.md")
        inputs.files(
...
tasks.matching { it.name == "preBuild" }.configureEach {
    dependsOn(generateUserGuides, *generateLicenceDocuments.toTypedArray())
}
