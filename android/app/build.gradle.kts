...
android {
    ...
    packaging {
        resources {
            ...
            // kotlinx-coroutines JARs) each ship the SAME pair of license-notice
            ...
            // template excludes them. LICENSE-COMPLIANCE NOTE: this removes only
            ...
            // copy of LICENSES/Apache-2.0.txt; see generateLicenseDocuments below).
...
// not go through `make` — failed because R.raw.usersguide / R.raw.license_* had no
...
// file per license — a plain copy, the SAME operation the Makefile rules and
// their check-guides verification perform, so the two build paths cannot
// disagree about the bytes. Until 0.83.0 both went through
// tools/render-copyright.py, which existed to CONCATENATE several inputs into
// one combined copyright.md; once that document was split into one verbatim
// license per file, nothing was left for it to do that Gradle's own Copy task
// does not, so it is gone.
//
//   license_gpl3.md    the app's own license, linked from the About screen
//   license_apache2.md required by Apache-2.0 §4(a) for the Apache-licensed
...
val generateLicenseDocuments = licenseDocuments.map { (name, source, what) ->
    tasks.register<Copy>("generate${name.replaceFirstChar { it.uppercase() }}Document") {
        description = "Copy res/raw/$name.md from $source ($what)."
        // The license sources live one level up from the android/ Gradle root, at
        // the repository root. `Copy` creates res/raw/ itself, which matters: git
        // tracks nothing in there, so after a fresh clone the directory is absent.
        val root = rootProject.projectDir.parentFile
        from(root.resolve(source))
...
tasks.matching { it.name == "preBuild" }.configureEach {
    dependsOn(generateUserGuides, *generateLicenseDocuments.toTypedArray())
}
...
kover {
    reports {
        ...
        //                   504 of 621 branches, 81.16%. Kover had been measuring
        ...
        //                   design: eight covered branches lost, or ten
        //                   uncovered ones added, take the figure under 80.
        //                   A figure quoted in prose goes stale the next time a
        //                   test lands; `make -C android cover-figures` prints
        //                   the current one, which is why the documents outside
        //                   this file now name the floor and that command
        //                   instead of a number.
